...
@Component({
  selector: 'app-zaposleni-meni-predmet-predavanja',
  templateUrl: './zaposleni-meni-predmet-predavanja.component.html',
  styleUrls: ['./zaposleni-meni-predmet-predavanja.component.css']
})
export class ZaposleniMeniPredmetPredavanjaComponent implements OnInit {
  ...
  obrisiFajlPredavanja(fajl) {
    this.predmetService.obrisiFajl(fajl.ime);
    this.predmetService.obrisiFajlPredmet(this.predmetIzabranSifra, "predavanja", fajl.ime).subscribe(res => {
      this.postaviPredmet();
    });
  }
  ...
  dodajFajlPredavanja() {
    ...
    this.predmetService.fajlUpload(this.imeFajla, this.formGroup.get('file').value, noviFajl, this.predmetIzabranSifra, "predavanja", null);

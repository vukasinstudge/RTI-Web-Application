...
export class PredmetLaboratorijaComponent implements OnInit {
  ...
  dohvatiSpiskove() {
    this.service.dohvatiSpiskoveStudent(this.predmetDetalji.sifra, "lab").subscribe((spiskovi: Spisak[]) => {
      if (spiskovi) {
      ...
      this.service.azurirajPrijavu(spisak, this.student.korime, this.noviFajl).subscribe(res => {
        if (res['poruka'] == 1) {
          if (this.noviFajl != null) {
            this.service.fajlUpload(this.imeFajla, this.formGroup.get('file').value, this.noviFajl, this.student.korime, "spiskoviLab", spisak);
          }

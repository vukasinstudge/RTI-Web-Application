...
@Component({
  selector: 'app-zaposleni-meni-predmet-vezbe',
  templateUrl: './zaposleni-meni-predmet-vezbe.component.html',
  styleUrls: ['./zaposleni-meni-predmet-vezbe.component.css']
})
export class ZaposleniMeniPredmetVezbeComponent implements OnInit {
  ...
  obrisiFajlVezbe(fajl) {
    this.predmetService.obrisiFajl(fajl.ime);
    this.predmetService.obrisiFajlPredmet(this.predmetIzabranSifra, "vezbe", fajl.ime).subscribe(res => {
      this.postaviPredmet();
    });
  }
  ...
  dodajFajlVezbe() {
    ...
    this.predmetService.fajlUpload(this.imeFajla, this.formGroup.get('file').value, noviFajl, this.predmetIzabranSifra, "vezbe", null);
